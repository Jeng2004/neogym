        ...
        stage('Copy files to Docker server') {
            steps {
                script {
                    ...
                    scp -i ~/.ssh/id_rsa -r /var/lib/jenkins/workspace/neogym/* root@54.163.183.128:~/neogym
                    '''
                }
            }
        }
        stage('Build Docker Image') {
            steps {
                script {
                    ...
                    sh '''
                    ssh -i ~/.ssh/id_rsa root@54.163.183.128 "cd ~/neogym && docker build -t neogym:latest ."
                    '''
                }
            }
        }
        stage('Create Docker Container') {
            steps {
                script {
                    ...
                    sh '''
                    ssh -i ~/.ssh/id_rsa root@54.163.183.128 "
